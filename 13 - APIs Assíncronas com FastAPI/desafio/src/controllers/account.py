...
# Define prefixo do recurso e exige autenticação via `login_required`
router = APIRouter(prefix="/accounts", dependencies=[Depends(login_required)])

# Serviços responsáveis por contas e transações
...
async def read_accounts(limit: int, skip: int = 0):
    # Lista contas com paginação simples (limit/skip)
...
async def create_account(account: AccountIn):
    # Cria uma nova conta e retorna seus dados
...
async def read_account_transactions(id: int, limit: int, skip: int = 0):
    # Lista transações associadas a uma conta específica
